...
def switch_tab(a, p, co, au):
    ...
    return {"tab-analysis": "analysis", "tab-portfolio": "portfolio",
...
def render_tab(tab, stored_results):
    if tab == "analysis":
        return _analysis_layout()
    elif tab == "portfolio":
        return _portfolio_layout()
    elif tab == "cost":
        return _cost_layout()
    elif tab == "audit":
    ...
    return html.Div([
        ...
        html.Div(style=CARD, children=[
            ...
            dcc.Upload(id="upload-data",
                children=html.Div([
                    html.Div("↑", style={"fontSize": "32px", "color": C["accent"], "marginBottom": "8px"}),
            ...
            html.Div(id="upload-status",
                     style={"marginTop": "12px", "fontSize": "13px", "color": C["muted"]}),
        ]),
        ...
        html.Div(id="column-mapping-section", style={"display": "none"}, children=[
            html.Div(style=CARD, children=[
                ...
                html.P("Map your CSV columns to PFEMS variables.",
                       style={"color": C["muted"], "fontSize": "13px", "marginBottom": "20px"}),
                dbc.Row([
                ...
                html.Div(style={"marginTop": "20px", "display": "flex",
                                "gap": "20px", "alignItems": "center", "flexWrap": "wrap"},
                         children=[
                ...
                html.Div(id="model-info-display",
                         style={"marginTop": "10px", "fontSize": "12px", "color": C["muted"]}),
            ]),
        ]),
        ...
        html.Div(id="charts-section", style={"display": "none"}, children=[
            ...
            html.Div(style=CARD, children=[
                dcc.Graph(id="importance-chart", figure=blank_fig("Run analysis to see feature importance"),
                          config={"displayModeBar": False}),
                html.Div(id="importance-text",
                         style={"marginTop": "8px", "fontSize": "12px", "color": C["muted"]}),
            ]),
            ...
            html.Div(style=CARD, children=[
                ...
                html.Div(style={"display": "flex", "gap": "12px", "flexWrap": "wrap"}, children=[
                    html.Button("↓ PDF REPORT", id="btn-pdf", n_clicks=0,
                                style={**BTN, "backgroundColor": C["danger"], "color": "#fff",
                                       "padding": "10px 20px"}),
                    html.Button("↓ EXCEL ALERT LOG", id="btn-excel", n_clicks=0,
                                style={**BTN, "backgroundColor": "#1D6F42", "color": "#fff",
                                       "padding": "10px 20px"}),
                    html.Button("↓ CSV RESIDUAL", id="btn-csv", n_clicks=0,
                                style={**BTN, "backgroundColor": C["accent3"], "color": "#fff",
                                       "padding": "10px 20px"}),
                ]),
...
def _portfolio_layout():
    return html.Div([
        html.Div(style=CARD, children=[
            html.P("PORTFOLIO OVERVIEW", style=HEAD),
            html.P("Upload multiple field datasets to compare monitoring status across assets.",
                   style={"color": C["muted"], "fontSize": "13px", "marginBottom": "20px"}),
            ...
            html.Div(id="portfolio-status",
                     style={"marginTop": "12px", "color": C["muted"], "fontSize": "13px"}),
        ]),
        html.Div(id="portfolio-table-section", style={"display": "none"}, children=[
            ...
            html.Div(style=CARD, children=[
                ...
                dcc.Graph(id="portfolio-chart", figure=blank_fig("Upload field data to compare"),
                          config={"displayModeBar": False}),
            ]),
        ]),
    ])
...
def _cost_layout():
    return html.Div([
        html.Div(style=CARD, children=[
            ...
            html.P("Compare PFEMS continuous monitoring against conventional methane "
                   "detection methods. Adjust parameters to match your field conditions.",
                   style={"color": C["muted"], "fontSize": "13px", "marginBottom": "24px"}),
            dbc.Row([
                dbc.Col([
                    html.Label("Daily production (MMscf/day)", style=LABEL_STYLE),
                    dcc.Input(id="cost-production", type="number", value=150,
                              min=1, max=500, step=1, style=INPUT_STYLE),
                ], md=3),
                dbc.Col([
                    html.Label("Methane price (USD per MMscf)", style=LABEL_STYLE),
                    dcc.Input(id="cost-price", type="number", value=3000,
                              min=100, max=20000, step=100, style=INPUT_STYLE),
                ], md=3),
                dbc.Col([
                    html.Label("Assumed leak rate (% of production)", style=LABEL_STYLE),
                    dcc.Input(id="cost-leak-rate", type="number", value=1.0,
                              min=0.1, max=10.0, step=0.1, style=INPUT_STYLE),
                ], md=3),
                dbc.Col([
                    html.Label("PFEMS annual licence fee (USD)", style=LABEL_STYLE),
                    dcc.Input(id="cost-pfems-fee", type="number", value=25000,
                              min=0, max=500000, step=1000, style=INPUT_STYLE),
                ], md=3),
            ], className="g-3"),
            html.Div(style={"marginTop": "20px"}, children=[
                html.Button("CALCULATE", id="btn-calculate-cost", n_clicks=0,
                            style={**BTN, "backgroundColor": C["accent"], "color": C["bg"]}),
            ]),
        ]),
...
def _audit_layout():
    ...
    if not log:
        return html.Div([
            html.Div(style=CARD, children=[
                ...
                html.Div("No analysis sessions recorded yet. Run an analysis to generate the first entry.",
                         style={"color": C["muted"], "fontSize": "13px"}),
            ]),
        ])
    hs = {"fontFamily": "monospace", "fontSize": "10px", "letterSpacing": "2px",
    ...
    cs = {"padding": "9px 12px", "fontSize": "12px", "color": C["text"],
          "borderBottom": "1px solid " + C["grid"], "whiteSpace": "nowrap"}
    cols = ["Ref","Timestamp","Field","Operator","Country","Points",
            "Alerts","MDLF%","CH4 Frac","Sigma","Cont%"]
    rows = []
    ...
    return html.Div([
        html.Div(style=CARD, children=[
            html.P("AUDIT LOG", style=HEAD),
            html.Div(str(len(log)) + " analysis sessions recorded. "
                     "Each entry is generated automatically when Run Analysis is clicked.",
